...
export class ListSiteComponent implements OnInit {
  ...
  getAllPostes() {
    this.siteService.getAllPostes().subscribe((data) => {
      this.postes = data;
    });
  }
  ...
  openEditDialog(site: Site): void {
    ...
    if (siteId !== undefined && !isNaN(siteId)) {
      // Récupérer les directions
      this.siteService.getDirections(siteId).subscribe((data: Direction[]) => {
        this.selectedDirections = data;  // Mettre à jour les directions sélectionnées
        console.log("Directions récupérées:", this.selectedDirections);
  
        // Récupérer les postes pour ce site
        this.siteService.getPostesBySiteId(siteId).subscribe((data: Poste[]) => {
          this.selectedPostes = data;  // Mettre à jour les postes sélectionnés
          console.log("Postes récupérés:", this.selectedPostes);
        }, error => {
      ...
      console.error("L'ID du site est invalide ou non défini");
    }
  }
